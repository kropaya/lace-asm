...
fn make_quantifier_block(quant: &str, vars: &Vec<&str>) -> &[u8] {
  ...
  for var in vars.iter() {
    let mut starchar = convert_utf8_to_starchar(var);
    let mut starlength = make_varint(starchar.length());
    output.append(starlength);
    output.append(starchar);
  }
  output.as_ref();
}
